...
class Square:
    ...
    def __init__(self, board, pos):
        ...
        compre1_pypsta = []
        for i_pypsta in range(3):
            compre1_pypsta.append(random.randrange(9223372036854775807))
        self.zobrist_strings = compre1_pypsta

    def set_neighbours(self):
        SIZE_pypsta_fdiv = SIZE
        (x, y) = (self.pos % SIZE, self.pos // SIZE_pypsta_fdiv)
        self.neighbours = []
        for for_target1_pypsta in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
            (dx, dy) = for_target1_pypsta
            (newx, newy) = (x + dx, y + dy)
...
class Board:
    ...
    def __init__(self):
        compre2_pypsta = []
        for pos_pypsta in range(SIZE * SIZE):
            compre2_pypsta.append(Square(self, pos_pypsta))
        self.squares = compre2_pypsta
    ...
    def useful_moves(self):
        compre3_pypsta = []
        for pos_pypsta in self.emptyset.empties:
            if self.useful(pos_pypsta):
                compre3_pypsta.append(pos_pypsta)
        return compre3_pypsta
    ...
    def __repr__(self):
        ...
        for y in range(SIZE):
            start = to_pos(0, y)
            compre4_pypsta = []
            for square_pypsta in self.squares[start:start + SIZE]:
                compre4_pypsta.append(SHOW[square_pypsta.color] + ' ')
            result.append(''.join(compre4_pypsta))
        return '\n'.join(result)
...
class UCTNode:
    ...
    def __init__(self):
        ...
        self.losses = 0
        compre5_pypsta = []
        for x_pypsta in range(SIZE * SIZE):
            compre5_pypsta.append(None)
        self.pos_child = compre5_pypsta
        self.parent = None
