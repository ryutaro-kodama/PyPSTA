...
def configuration(cell, passkey=None):
    ...
    if not cell.encrypted:
        return {'wireless-essid': cell.ssid, 'wireless-channel': 'auto'}
    elif cell.encryption_type.startswith('wpa'):
        if len(passkey) != 64:
            passkey = PBKDF2(passkey, cell.ssid, 4096).hexread(32)
        return {'wpa-ssid': cell.ssid, 'wpa-psk': passkey, 'wireless-channel': 'auto'}
    elif cell.encryption_type == 'wep':
        ascii_lengths = (5, 13, 16, 29)
        if len(passkey) in ascii_lengths:
            passkey = 's:' + passkey
        return {'wireless-essid': cell.ssid, 'wireless-key': passkey}
    else:
        raise NotImplementedError
bound_ip_re = re.compile('^bound to (?P<ip_address>\\S+)', flags=re.MULTILINE)
...
class Scheme(object):
    ...
    def for_file(cls, interfaces):
        ...
        """
        return type(cls)(cls.__name__, (cls,), {'interfaces': interfaces})
    ...
    def __str__(self):
        ...
        compre1_pypsta = []
        for for_target1_pypsta in self.options.items():
            (k_pypsta, v_pypsta) = for_target1_pypsta
            compre1_pypsta.append('\n    {k} {v}'.format(k=k_pypsta, v=v_pypsta))
        options = ''.join(compre1_pypsta)
    ...
    def as_args(self):
        compre2_pypsta = []
        for for_target2_pypsta in self.options.items():
            (k_pypsta, v_pypsta) = for_target2_pypsta
            compre2_pypsta.append(('-o', '{k}={v}'.format(k=k_pypsta, v=v_pypsta)))
        args = list(itertools.chain.from_iterable(compre2_pypsta))
...
scheme_re = re.compile('iface\\s+(?P<interface>[^-]+)(?:-(?P<name>\\S+))?')
    ...
    while lines:
        ...
        if match:
            ...
            while lines and lines[0].startswith(' '):
                (key, value) = re.sub('\\s{2,}', ' ', lines.pop(0).strip()).split(' ', 1)
                options[key] = value
