...
cells_re = re.compile('Cell \\d+ - ')
quality_re_dict = {'dBm': re.compile('Quality[=:](?P<quality>\\d+/\\d+).*Signal level[=:](?P<siglevel>-\\d+) dBm?(.*Noise level[=:](?P<noiselevel>-\\d+) dBm)?'), 'relative': re.compile('Quality[=:](?P<quality>\\d+/\\d+).*Signal level[=:](?P<siglevel>\\d+/\\d+)'), 'absolute': re.compile('Quality[=:](?P<quality>\\d+).*Signal level[=:](?P<siglevel>\\d+)')}
frequency_re = re.compile('^(?P<frequency>[\\d\\.]+ .Hz)(?:[\\s\\(]+Channel\\s+(?P<channel>\\d+)[\\s\\)]+)?$')
identity = lambda x: x
key_translations = {'encryption key': 'encrypted', 'essid': 'ssid'}

def normalize_key(key):
    key = key.strip().lower()
    key = key_translations.get(key, key)
    return key.replace(' ', '')
normalize_value = {'ssid': lambda v: v.strip('"'), 'encrypted': lambda v: v == 'on', 'address': identity, 'mode': identity, 'channel': int}
...
def normalize(cell_block):
    ...
    while lines:
        ...
        if line.startswith('Quality'):
            for for_target1_pypsta in quality_re_dict.items():
                (re_name, quality_re) = for_target1_pypsta
