...
def fuzzy_find_cell(interface, query):
    ...
    compre1_pypsta = []
    for cell_pypsta in matches:
        compre1_pypsta.append(cell_pypsta.ssid)
    num_unique_matches = len(set(compre1_pypsta))
...
def scan_command(args):
    compre2_pypsta = []
    for cell_pypsta in Cell.all(args.interface):
        compre2_pypsta.append([cell_pypsta.signal, cell_pypsta.ssid, 'protected' if cell_pypsta.encrypted else 'unprotected'])
    print_table(compre2_pypsta)
...
def show_command(args):
    pypsta_stared_arg_temp1 = get_scheme_params(args.interface, args.scheme, args.ssid)
    scheme = Scheme.for_file(args.file).for_cell(*pypsta_stared_arg_temp1, pypsta_stared_arg1=pypsta_stared_arg_temp1)
    print(scheme)
...
def add_command(args):
    ...
    pypsta_stared_arg_temp2 = get_scheme_params(args.interface, args.scheme, args.ssid)
    scheme = scheme_class.for_cell(*pypsta_stared_arg_temp2, pypsta_stared_arg2=pypsta_stared_arg_temp2)
    scheme.save()
...
def connect_command(args):
    ...
    if args.adhoc:
        ...
        pypsta_stared_arg_temp3 = get_scheme_params(args.interface, 'adhoc', args.scheme)
        scheme = scheme_class.for_cell(*pypsta_stared_arg_temp3, pypsta_stared_arg3=pypsta_stared_arg_temp3)
    else:
...
def autoconnect_command(args):
    compre3_pypsta = []
    for cell_pypsta in Cell.all(args.interface):
        compre3_pypsta.append(cell_pypsta.ssid)
    ssids = compre3_pypsta
    for_else_iter1_pypsta = pypsta_iter(Scheme.all())
    for_else_target2_pypsta = pypsta_next(for_else_iter1_pypsta, None)
    while for_else_target2_pypsta is not None:
        scheme = for_else_target2_pypsta
...
def arg_parser():
    ...
    parser.add_argument('-i', '--interface', default='wlan0', help='Specifies which interface to use (wlan0, eth0, etc.)')
    parser.add_argument('-f', '--file', default='/etc/network/interfaces', help='Specifies which file for scheme storage.')
    ...
    compre4_pypsta = []
    for scheme_pypsta in Scheme.all():
        compre4_pypsta.append(scheme_pypsta.name)
    parser_connect.get_options = lambda : compre4_pypsta
    parser_autoconnect = subparsers.add_parser('autoconnect', help='Searches for saved schemes that are currently available and connects to the first one it finds.')
